...
export class AdsComponent implements OnInit, OnDestroy {
  ...
  toggleAdStatus(ad: AdDto): void {
    ...
    this.adsService.updateAdStatus(ad.id, newStatus)
  ...
  deleteAd(): void {
    ...
    this.adsService.deleteAd(this.adToDelete)
      .pipe(
        takeUntil(this.destroy$),
      ...
      .subscribe({
        next: (response) => {
          if (response.isSuccess) {
            ...
            this.selectedAds = this.selectedAds.filter(id => id !== this.adToDelete);
            this.applyFilters();
            this.showDeleteConfirm = false;
            this.adToDelete = null;
            this.successMessage = response.message || 'Ad deleted successfully!';
        ...
        error: (error) => {
          ...
          if (error.status === 404) {
            ...
            this.error = error.error?.message || error.message || 'Failed to delete ad.';
          }
          this.showDeleteConfirm = false;
          this.adToDelete = null;
        }
    ...
    forkJoin(deleteObservables)
      ...
      .subscribe({
        next: (responses) => {
          ...
          const failedDeletions = responses.filter(response => !response.isSuccess);
          
          if (failedDeletions.length === 0) {
            // All deletions successful
            this.ads = this.ads.filter(ad => !this.selectedAds.includes(ad.id));
            this.selectedAds = [];
            this.applyFilters();
            ...
            const failedIds = failedDeletions.map((_, index) => this.selectedAds[index]);
            this.ads = this.ads.filter(ad => !failedIds.includes(ad.id));
            this.selectedAds = this.selectedAds.filter(id => !failedIds.includes(id));
            this.applyFilters();
            
            const errorMessages = failedDeletions.map(response => response.message).join(', ');
        ...
        error: (error) => {
          ...
          if (error.status === 403) {
            this.error = 'You are not authorized to delete some of the selected ads.';
          } else if (error.status === 500) {
            this.error = 'Server error occurred while deleting ads.';
          } else {
            this.error = error.error?.message || error.message || 'Failed to delete selected ads.';
          }
        }
      });
  }
  ...
  updateAd(): void {
    ...
    if (this.adToUpdate.adFieldDtos && this.adToUpdate.adFieldDtos.length > 0) {
      console.log('DynamicFieldValues before conversion:', this.adToUpdate.adFieldDtos);
    ...
    this.adsService.updateAd(this.adToUpdate.id, formData)
      .pipe(
        takeUntil(this.destroy$),
      ...
      .subscribe({
        ...
        error: (error: any) => {
          ...
          this.updateError = error.error?.message || error.message || 'Failed to update ad. Please try again.';
        }
      });
  }
